...
- Use `*_test.go` files with `BenchmarkXxx` funcs.
- Put benchmarks near tests for the same package.
- Name clearly: `BenchmarkMarshalSmall`, `BenchmarkMarshalLarge`, `BenchmarkGetHit`, `BenchmarkGetMiss`, etc.

Example layout:
```text
pkg/foo/
  foo.go
  foo_test.go        # tests + benchmarks are ok together
  foo_bench_test.go  # optional if you want separation
```
---
